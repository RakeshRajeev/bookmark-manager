...
def get_total_clicks(db: Session):
    return db.query(models.Bookmark).with_entities(models.Bookmark.clicks).all()

def get_popular_bookmarks(db: Session, limit: int = 5):
    return db.query(models.Bookmark).order_by(models.Bookmark.clicks.desc()).limit(limit).all()
